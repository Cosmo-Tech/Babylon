...
def get(api_url: str,
    ...
    """Get a workspace from the organization"""
    response = oauth_request(f"{api_url}/organizations/{organization_id}/workspaces/{workspace_id}", azure_token)
    if response is None:
        return CommandResponse.fail()
    workspace = response.json()
    if filter:
        workspace = jmespath.search(filter, workspace)

...
@pass_dt_client
@argument(
    "model_file", type=click.Path(exists=True, file_okay=True, dir_okay=False, readable=True, path_type=pathlib.Path)
)
@option("-o", "--override", "override_if_exists", is_flag=True, help="Override existing models")
@allow_dry_run
def upload(dt_client: DigitalTwinsClient, model_file: pathlib.Path, override_if_exists: bool, dry_run: bool):

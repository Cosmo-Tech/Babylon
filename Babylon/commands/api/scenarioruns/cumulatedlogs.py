...
def cumulatedlogs(
    ...
    logger.info(f"Getting cumulated logs for scenariorun: 'scenariorun_id'")  # {context['scenariorun_id']}
    ...
    response = service.cumulatedlogs()

    if response is None:
        return CommandResponse.fail()

    cumulated_run_logs = response.json()

    return CommandResponse.success(cumulated_run_logs, verbose=True)

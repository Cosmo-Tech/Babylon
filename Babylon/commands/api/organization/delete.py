...
def delete(api_url: str, azure_token: str, organization_id: str, force_validation: bool = False) -> CommandResponse:
    """Delete a dataset from the organization"""
    if not force_validation and not confirm_deletion("organization", organization_id):
        return CommandResponse.fail()
    response = oauth_request(f"{api_url}/organizations/{organization_id}", azure_token, type="DELETE")
    if response is None:
        return CommandResponse.fail()

...
class AzurePowerBIDatasetService:
    ...
    def update_credentials(self, workspace_id: str, dataset_id: str):
        ...
        for datasource in output_data:
            ...
            if response is None:
                logger.error(f"[powerbi] could not update credentials of datasource {datasource_id}")
                continue
            logger.info(f"[powerbi] successfully updated credentials of datasource {datasource_id}")

...
class WorkingDir:
    ...
    def compare_to_template(self, update_if_error: bool = False) -> bool:
        ...
        log_level = logging.INFO if update_if_error else logging.ERROR
        has_err = False
        ...
        for root, dirs, files in os.walk(self.template_path):
            ...
            if not local_dir_path.exists() and not (rel_path == pathlib.Path(".") and self.is_zip):
                logger.log(log_level, f"MISSING DIR: {local_dir_path}")
                has_err = True
                if update_if_error and not self.is_zip:
                    logger.log(log_level, "CORRECTION :  Creating it")
            ...
            for _f in files:
                ...
                if os.path.getsize(_root / f_rel_path) > -1:
                    ...
                    if not local_file_path.exists():
                        logger.log(log_level, f"MISSING FILE: {local_file_path}")
                        has_err = True
                        if update_if_error and not self.is_zip:
                            logger.log(log_level, "CORRECTION:   Copying it from template")
                        ...
                        if missing_keys:
                            has_err = True
                            logger.log(log_level, f"YAML: In file {local_file_path}")
                            logger.log(log_level, "            The following keys are missing :")
                            for _k in missing_keys:
                                logger.log(log_level, f"            - {_k}")
                            if update_if_error and not self.is_zip:
                                logger.log(log_level, "CORRECTION: Adding them")
